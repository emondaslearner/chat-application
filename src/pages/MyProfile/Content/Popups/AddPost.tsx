...
const FileUploadCom: React.FC<FileUploadComProps> = ({ setFileStatus }) => {
  return (
    <div
      onClick={(e) => {
        e.stopPropagation();
      ...
      <div className="max-h-[80vh] p-5 relative">
        ...
        {fileStatus && files.length === 0 && (
          <DragFile
            name="file"
            multiple={true}
            children={<FileUploadCom setFileStatus={setFileStatus} />}
            ...
            <div className="h-[100%] overflow-y-auto">
              ...
              {filesUrls.map((data: string, index: number) => {
                return (
                  <div
                    key={index}
                    className={`relative ${index > 0 && "mt-[10px]"} border-[1px] border-light_border_ dark:border-dark_border_`}
                  >
        ...
        >
          ...
          <div className="flex items-center gap-x-2">
            <div title="Photos/videos" className="p-[10px] rounded-full dark:bg-dark_gray_ cursor-pointer bg-dark_gray_ table mx-auto">
              <FaCloudUploadAlt size={20} />
            </div>

            <div title="Emoji" className="p-[10px] rounded-full dark:bg-dark_gray_ cursor-pointer bg-dark_gray_ table mx-auto">
              <MdOutlineEmojiEmotions size={20} />
            </div>

            <div title="Background" className="p-[20px] rounded-full cursor-pointer table mx-auto bg-green-500"></div>
          </div>
        </div>
      </div>
    </Modal>
